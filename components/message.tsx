        ...
        >
          ...
          {message.parts?.map((part, index) => {
            ...
            if (type === "reasoning" && part.text?.trim().length > 0) {
              const reasoningLines = part.text
                .split("\n")
                .filter((line: string) => line.trim());
              return (
                <ChainOfThought defaultOpen={isLoading} key={key}>
                  <ChainOfThoughtHeader>
                    {isLoading ? "Pensando..." : "Pensó por un momento"}
                  </ChainOfThoughtHeader>
                  <ChainOfThoughtContent>
                    {reasoningLines
                      .slice(0, 5)
                      .map((line: string, idx: number) => (
                        <ChainOfThoughtStep
                          ...
                          status={
                            isLoading && idx === reasoningLines.length - 1
                              ? "active"
                              : "complete"
                          }
                        />
            ...
            if ((type as string) === "tool-invocation") {
              ...
              return (
                <Tool className="group" key={key}>
                  <ToolHeader
                    state={toolPart.toolInvocation.state}
                    title={toolPart.toolInvocation.toolName}
                    type="tool-invocation"
                  />
                  <ToolContent>
                    <ToolInput input={toolPart.toolInvocation.args} />
                    {"result" in toolPart.toolInvocation && (
                      <ToolOutput
                        errorText={undefined}
                        output={toolPart.toolInvocation.result}
                      />
                    )}
                  </ToolContent>
                </Tool>
...
export const ThinkingMessage = () => {
  const { agentStatus } = useDataStream();
  ...
  const label = statusLabels[agentStatus] || "Pensando";
